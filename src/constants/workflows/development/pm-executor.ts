...
const WORKFLOW_CONFIG = `# PM Executor - 티켓 실행 워크플로우
name: "pm-executor"
...
const INSTRUCTIONS = `# PM Executor 지시사항 (자율 실행 모드)
...
<action>필수 파일 존재 확인:
  ...
  2. {epics_folder}/ - Epic별 통합 티켓 파일 (## TICKET 마크다운 섹션 포함)
...
<check if="progress file not exists">
  <action>새로운 실행 - execution-progress.md 명시적 초기화</action>
...
<action>현재 Wave의 모든 티켓들 로드 및 파싱
...
<step n="8" goal="Wave 완료 및 코드 리뷰 대기">

<action>⚠️ CRITICAL: workflow_state 업데이트
...
<action>🔴 **CRITICAL: CLAUDE.md 업데이트**

경로: {{claude_context_file}} = {{project-root}}/CLAUDE.md

Wave 완료 후 반드시 CLAUDE.md 업데이트:
- 진행 상황
- 실행 환경
- 발견된 패턴 & 컨벤션
- 알려진 이슈 & 해결책
...
export const PM_EXECUTOR_METADATA = {
  id: 'pm-executor',
  title: 'PM Executor',
  description: '티켓을 Wave 순서대로 자율 실행하여 프로젝트 구현',
