...
agent:
  metadata:
    id: .anyon/agents/{agent-name}/{agent-name}.md
    name: 'Persona Name'
    title: 'Agent Title'
    icon: 'emoji'
    type: simple
...
- **id**: Final compiled path (`.anyon/agents/{name}/{name}.md` for standalone)
...
Questions asked during `anyon agent-install`:
...
See: `src/modules/anyon-builder/reference/agents/simple-examples/commit-poet.agent.yaml`
...
cp /path/to/commit-poet.agent.yaml .anyon/custom/agents/

# Install with personalization
anyon agent-install
# or: npx anyon agent-install
```
