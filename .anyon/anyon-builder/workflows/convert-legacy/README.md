...
The Convert Legacy workflow is a comprehensive migration tool that converts ANYON v4 items (agents, workflows, modules) to v6 compliant format with proper structure and conventions. It bridges the gap between legacy ANYON implementations and the modern v6 architecture, ensuring seamless migration while preserving functionality and improving structure.
...
- Prompts for target module (anyon-method, anyon-builder, cis, custom)
- Determines proper installation paths using v6 conventions
- Shows target location for user confirmation
- Ensures all paths use `{project-root}/.anyon/` format
...
- Maps `elicit: true` flags to `<invoke-task halt="true">{project-root}/.anyon/core/tasks/advanced-elicitation.xml</invoke-task>` tags
...
- **Solution**: Ensure all references use `{project-root}/.anyon/` format
...
- Review the workflow creation guide at `/.anyon/anyon-builder/workflows/create-workflow/workflow-creation-guide.md`
- Check conversion mappings at `/.anyon/anyon-builder/data/v4-to-v6-mappings.yaml`
- Validate output using `checklist.md`
- Consult ANYON v6 documentation for proper conventions

---

_Part of the Anyon Method v6 - BMB (Builder) Module_
