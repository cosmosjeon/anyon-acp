...
<critical>You MUST have already loaded and processed: {project-root}/.anyon/anyon-builder/workflows/create-agent/workflow.yaml</critical>
...
<step n="1" goal="Optional brainstorming for agent ideas">
  ...
  <check if="user answered yes">
    <action>Invoke brainstorming workflow: {project-root}/.anyon/core/workflows/brainstorming/workflow.yaml</action>
...
**Reference:** See {project-root}/.anyon/anyon-builder/docs/understanding-agent-types.md for "The Same Agent, Three Ways" example.
...
Explain: "Most ANYON v6 workflows default to **intent-based + medium/high interactivity**
...
- trigger: [another capability]
  workflow: "{project-root}/.anyon/SOURCE_MODULE/workflows/path/to/workflow.yaml"
  workflow-install: "{project-root}/.anyon/THIS_MODULE/workflows/vendored/path/workflow.yaml"
...
<example type="yaml">
  agent:
    metadata:
      id: .anyon/{{target_module}}/agents/{{agent_filename}}.md
...
<step n="11" goal="Handle build tools availability">
  <action>Check if ANYON build tools are available in this project</action>

  <check if="ANYON-METHOD project with build tools">
...
1. Run the ANYON Method installer to this project location
