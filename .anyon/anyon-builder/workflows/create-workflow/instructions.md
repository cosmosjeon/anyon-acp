...
<critical>You MUST have already loaded and processed: {project-root}/.anyon/anyon-builder/workflows/create-workflow/workflow.yaml</critical>
...
- Workflow: {project-root}/.anyon/core/workflows/brainstorming/workflow.yaml
...
- What module will it belong to? (e.g., "anyon-method", "anyon-builder", "cis")
...
- Author name (default to user_name or "Anyon")
...
config_source: '{project-root}/.anyon/{{target_module}}/config.yaml'
...
# Critical variables from config
config_source: '{project-root}/.anyon/module/config.yaml'
output_folder: '{config_source}:output_folder'
user_name: '{config_source}:user_name'
communication_language: '{config_source}:communication_language'
date: system-generated
...
1. ALWAYS include critical headers:
   - Workflow engine reference: {project-root}/.anyon/core/tasks/workflow.xml
...
4. Use proper XML tags from guide:
   ...
   - Output: <template-output>, <invoke-task halt="true">{project-root}/.anyon/core/tasks/advanced-elicitation.xml</invoke-task>, <critical>, <example>
...
- File paths use .anyon/ root (not {project-root})

<action>Configure web_bundle section in workflow.yaml:</action>

1. Copy core workflow metadata (name, description, author)
2. Convert all file paths to .anyon/-relative paths:
   - Remove {project-root}/ prefix
   - Remove {config_source} references (use hardcoded values)
   - Example: "{project-root}/.anyon/anyon-method/workflows/x" → ".anyon/anyon-method/workflows/x"
...
3. List ALL referenced files by scanning:
   ...
   **Critical: Workflow Dependencies**
   ...
   - Example: `<invoke-workflow>{project-root}/.anyon/core/workflows/x/workflow.yaml</invoke-workflow>`
     → Add ".anyon/core/workflows/x/workflow.yaml" to web_bundle_files
...
web_bundle:
  ...
  existing_workflows:
    - workflow_variable_name: '.anyon/path/to/workflow.yaml'
```
...
web_bundle:
  ...
  author: 'Anyon'
  instructions: '.anyon/anyon-method/workflows/brainstorm-game/instructions.md'
  template: false
  web_bundle_files:
    - '.anyon/anyon-method/workflows/brainstorm-game/instructions.md'
    - '.anyon/mmm/workflows/brainstorm-game/game-context.md'
    - '.anyon/core/workflows/brainstorming/workflow.yaml'
  existing_workflows:
    - core_brainstorming: '.anyon/core/workflows/brainstorming/workflow.yaml'
```
...
- Next steps:
  - Run the ANYON Method installer to this project location
