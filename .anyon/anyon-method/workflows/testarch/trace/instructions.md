...
1. Load relevant knowledge fragments from `{project-root}/.anyon/anyon-method/testarch/tea-index.csv`:
...
3. Read related Anyon artifacts (if available):
...
4. **Load supporting artifacts**:
   ...
   - `anyon-workflow-status.md` → Workflow completion status (if `check_all_workflows_complete: true`)
...
- Traceability Matrix: `.anyon/output/traceability-matrix.md`
- Test Design: `.anyon/output/test-design-epic-2.md`
- Test Results: `ci-artifacts/test-report-2025-01-15.xml`
- NFR Assessment: `.anyon/output/nfr-assessment-release-1.2.md`
```
...
1. **Update workflow status** (if `append_to_history: true`):
   - Append gate decision to `anyon-workflow-status.md` under "Gate History" section
   - Format:
     ...
     - **Document**: [gate-decision-story-1.3.md](.anyon/output/gate-decision-story-1.3.md)
...
2. **Generate stakeholder notification** (if `notify_stakeholders: true`):
   ...
   Full Report: .anyon/output/gate-decision-story-1.3.md
   ```
...
## Integration with Anyon Artifacts
...
- ✅ Workflow status updated (anyon-workflow-status.md)
