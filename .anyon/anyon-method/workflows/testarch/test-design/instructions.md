...
**Workflow ID**: `.anyon/anyon-method/testarch/test-design`
...
1. **Check for sprint-status.yaml**
   - If `{output_folder}/anyon-sprint-status.yaml` exists → **Epic-Level Mode** (Phase 4)
   - If NOT exists → Check workflow status

2. **Check workflow-status.yaml**
   - Read `{output_folder}/anyon-workflow-status.yaml`
