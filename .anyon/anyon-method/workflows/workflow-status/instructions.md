...
<critical>You MUST have already loaded and processed: {project-root}/.anyon/anyon-method/workflows/workflow-status/workflow.yaml</critical>
...
<action>Search {output_folder}/ for file: anyon-workflow-status.yaml</action>
...
<action>Read anyon-workflow-status.yaml</action>
...
**Command:** /anyon:anyon-method:workflows:{{next_workflow_id}}
...
<check if="choice == 3">
  <action>Display complete anyon-workflow-status.yaml file contents</action>
</check>
...
<action>Read {output_folder}/anyon-workflow-status.yaml if exists</action>
...
<template-output>status_file_path = {{path to anyon-workflow-status.yaml}}</template-output>
</check>
...
<action>Check if {output_folder}/anyon-workflow-status.yaml exists</action>
...
<action>Read {output_folder}/anyon-workflow-status.yaml</action>
