...
<critical>This workflow handles BOTH new projects AND legacy projects being migrated to Anyon Method</critical>

<workflow>

<step n="1" goal="Scan for existing work">
<output>Welcome to Anyon Method, {user_name}!</output>
...
To check progress: Load any BMM agent and run /anyon:anyon-method:workflows:workflow-status
...
2. **Anyon Method** - Full planning for complex projects
...
**2. Anyon Method** 🎯 {{#if recommended}}(RECOMMENDED){{/if}}
...
<output>Your Anyon workflow path:
...
<check if="y">
  ...
  <action>Save to {output_folder}/anyon-workflow-status.yaml</action>
  <action>Identify next workflow and agent</action>

<output>✅ **Created:** {output_folder}/anyon-workflow-status.yaml
...
To check progress: /anyon:anyon-method:workflows:workflow-status
