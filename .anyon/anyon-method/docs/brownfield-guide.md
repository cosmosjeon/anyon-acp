# Anyon Method Brownfield Development Guide
...
| **Anyon Method**       | Feature sets, integrations | 10-50+          | Integrate with existing architecture            |
...
- Keywords: "fix", "bug" → Quick Flow, "dashboard", "platform" → Anyon Method, "enterprise", "multi-tenant" → Enterprise Method
...
```
System: "Found PRD.md (Anyon Method track, 30 stories, 6 months old)"
...
1. **First:** Run `shard-doc` tool to split large files:
   ...
   ```bash
   # Load Anyon Master or any agent
   .anyon/core/tools/shard-doc.xml --input docs/massive-doc.md
   ```
...
2. **Then:** Run `index-docs` task to create navigation:
   ...
   ```bash
   .anyon/core/tasks/index-docs.xml --directory ./docs
   ```
...
.anyon/core/tools/shard-doc.xml \
...
- **PRD** (Anyon Method) can't reference existing code → Designs incompatible features
...
- `product-brief` - Strategic planning (Anyon Method/Enterprise tracks only)
...
**Anyon Method/Enterprise:** Use `prd` workflow
...
### Phase 3: Solutioning (Anyon Method/Enterprise Only)
...
- `create-architecture` - Extend architecture docs (Anyon Method/Enterprise)
- `create-epics-and-stories` - Create epics and stories AFTER architecture
- `implementation-readiness` - Validate before implementation (Anyon Method/Enterprise)
...
### Scenario 3: Feature Set (Anyon Method)
...
### Scenario 4: Complex Integration (Anyon Method)
...
product-brief           # Strategic planning (Anyon Method/Enterprise only)

# Phase 2: Planning (Required)
# PM agent:
tech-spec               # Quick Flow track
prd                     # Anyon Method/Enterprise tracks

# Phase 3: Solutioning (Anyon Method/Enterprise)
# Architect agent:
...
- `docs/anyon-workflow-status.yaml` - Progress tracker

**Phase 2 Planning:**

- `docs/tech-spec.md` (Quick Flow track)
- `docs/PRD.md` (Anyon Method/Enterprise tracks - FRs/NFRs only)
...
- `docs/architecture.md` (Anyon Method/Enterprise tracks)
...
flowchart TD
    ...
    TRACK -->|Anyon Method| PRD[prd → architecture]
...
5. ✅ **Start with simpler track if unsure** - Easy to upgrade (Quick Flow → Anyon Method)
...
- [GitHub Issues](https://github.com/anyon-code-org/ANYON-METHOD/issues)
- [YouTube Channel](https://www.youtube.com/@AnyonCode)
